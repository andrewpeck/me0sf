...
def work_chamber(chamber_data, patlist, NUM_PARTITIONS=8, MAX_SPAN=37, WIDTH=192,group_width=8,ghost_width=4,NUM_VALS=4):
    ...
    def priority_encoder(group_vals,num_output=NUM_VALS):
        ...
        for i in range(len(group_vals)):
            ...
            ly_c=group_vals[i][1]
            quality=partition|(strip<<3)|(pat_id<<(3+9))|(ly_c<<(3+9+4))
    ...
    for i in range(NUM_PARTITIONS):
        partition_dat=work_partition(chamber_data=chamber_data[i],patlist=patlist,MAX_SPAN=MAX_SPAN,WIDTH=WIDTH,group_width=group_width,ghost_width=ghost_width)
