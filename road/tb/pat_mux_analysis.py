...
def run_pat_mux_analysis(patlist,MAX_SPAN=37):
    titles_iddiscrepancies=['Testcase #','Strip','Parsed Data','Pat_unit_mux ID','Emulator ID']
    titles_cntdiscrepancies=['Testcase #','Strip','Parsed Data','Pat_unit_mux CNT','Pat_unit_mux ID','Emulator CNT','Emulator ID']
    ...
    if (proceed=='Y' or proceed=='y'):
        ...
        with open('discrepancies_id.csv','r') as csv_file:
                ...
                csv_file.close()
        num_testcases=int(input('How many ID testcases would you like to analyze? Choose up to %d. '%max_testcase_num))
        ...
        with open('discrepancies_cnt.csv','r') as csv_file:
            ...
            max_testcase_num=max(testcase_nums)
            csv_file.close()
        num_testcases=int(input('How many CNT testcases would you like to analyze? Choose up to %d. '%int(max_testcase_num)))
        ...
        with open ('id_discrep_parsed.csv','r') as csv_file:
            ...
            for line in csv_reader:
                ...
                printly_dat(data=literal_eval(line[2]),mask=mask_t,MAX_SPAN=MAX_SPAN) #FIGURE OUT WHY THIS IS A PROBLEM
                print('\n\n')
        ...
        if (proceed=='Y' or proceed=='y'):
            ...
            with open('discrepancies_id.csv','r') as csv_file1:
                ...
                with open('id_discrep_parsed.csv','w') as csv_file2:
                    csv_writer=csv.writer(csv_file2)
                    csv_writer.writerow(['Testcase #','Parsed Data','Pat_unit_mux ID','Emulator ID'])
                    for line in csv_reader:
                        if (int(line[0])<=int(num_testcases)):
                            csv_writer.writerow(line)
                    csv_file2.close()
            ...
            proceed=input('Would you like to analyze some of the CNT discrepancies? Y/N ')
            if (proceed=='N' or proceed=='n'):
                ...
                with open('discrepancies_cnt.csv','r') as csv_file:
                    ...
                    max_testcase_num=max(testcase_nums)
                    csv_file.close()
                num_testcases=int(input('How many CNT testcases would you like to analyze? Choose up to %d. '%max_testcase_num))
                #parse data for CNT discrepancies
                with open('discrepancies_cnt.csv','r') as csv_file1:
                    csv_reader=csv.reader(csv_file1)
                    next(csv_reader)
                    with open('cnt_discrep_parsed.csv','w') as csv_file2:
                        csv_writer=csv.writer(csv_file2)
                        csv_writer.writerow(['Testcase #','Parsed Data','Pat_unit_mux CNT','Pat_unit_mux ID','Emulator CNT','Emulator ID'])
                        for line in csv_reader:
                            if (int(line[0])<=int(num_testcases)):
                                csv_writer.writerow(line)
                        csv_file2.close()
