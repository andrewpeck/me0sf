...
async def chamber_test(dut):
    ...
    MAX_SPAN = max(
        [
            dut.partition_gen[0].partition_inst.pat_unit_mux_inst.LY0_SPAN.value,
    ...
    for i in range(len(dut.partition_gen[0].partition_inst.pat_unit_mux_inst.PATLIST)):
        ...
        ly0_hi = (
            dut.partition_gen[0]
            .partition_inst.pat_unit_mux_inst.PATLIST[i]
            .ly0.hi.value
        )
        ly0_lo = (
            dut.partition_gen[0]
    ...
    for j in range(1):
        ...
        chamber_data = [
            datadev_mux(WIDTH=width, track_num=4, nhit_hi=10, nhit_lo=3)
            for i in range(8)
        ]
        # chamber_data=[[0]*6]*8
        queue.append(chamber_data)
        ...
        for r in range(len(partition_nums)):
            ...
            print(
                "Firmware layer count: %d" % int(dut.segs_o[r].strip.pattern.cnt.value)
            )
