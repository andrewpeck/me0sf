...
def analysis(root_dat, hits, bx, bx_list, cross_part, verbose, pu, num_or):
    ...
    st_effi_eta_high_passed_pt = ROOT.TH1F("st_effi_eta_high_pass_pt", "st_effi_eta_high_pass_pt",14, array('d',bins))
    st_effi_eta_high_total_pt = ROOT.TH1F("st_effi_eta_high_total_pt", "st_effi_eta_high_total_pt",14, array('d',bins))
    st_effi_eta_low_passed_pt = ROOT.TH1F("st_effi_eta_low_passed_pt", "st_effi_eta_low_passed_pt",14, array('d',bins))
    st_effi_eta_low_total_pt = ROOT.TH1F("st_effi_eta_low_total_pt", "st_effi_eta_low_total_pt",14, array('d',bins))
    st_effi_passed_pt_eta = ROOT.TH2F("st_effi_passed_pt_eta", "st_effi_passed_pt_eta",14, array('d',bins), 8, 0.5, 8.5)
    st_effi_total_pt_eta = ROOT.TH2F("st_effi_total_pt_eta", "st_effi_total_pt_eta",14, array('d',bins), 8, 0.5, 8.5)
    ...
    for (ievent, event) in enumerate(root_dat):
        ...
        # read digihit info
        digihit_region_orig = event["me0_digi_hit_region_i"]
        digihit_chamber_orig = event["me0_digi_hit_chamber_i"] - 1
        digihit_eta_partition_orig = event["me0_digi_hit_eta_partition_i"] - 1
        digihit_layer_orig = event["me0_digi_hit_layer_i"] - 1
        digihit_strip_orig = event["me0_digi_hit_strip_i"]
