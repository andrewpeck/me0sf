...
def analysis(root_dat, hits, bx, bx_list, cross_part, verbose, pu, num_or):
    ...
    for (ievent, event) in enumerate(root_dat):
        ...
        for chamber in online_segment_chamber: 
            for seg in online_segment_chamber[chamber]:
                ...
                if seg.partition % 2 != 0:
                    seg_eta.append((seg.partition // 2))
                    seg_eta.append((seg.partition // 2) + 1)
                    seg.partition = (seg.partition // 2) + 1
                else:
                    seg_eta.append((seg.partition // 2))
                    seg.partition = (seg.partition // 2)
    ...
    c1.SetGrid()
    c1.DrawFrame(0, 0, 9, 0.05, "#eta PartitionAverage Fraction per VFAT")
    ...
    me0_h_seg_rate_frac_eta_partition.SetMarkerColor(kBlack)
    me0_h_seg_rate_frac_eta_partition.SetLineWidth(2)
    me0_h_seg_rate_frac_eta_partition.SetLineColor(kBlack)
    ...
    c3.SetGrid()
    c3.DrawFrame(0, 0, 9, 0.02, "#eta PartitionAverage Fraction per VFAT")
    ...
    me0_h_seg_rate_frac_vfat_window.SetMarkerColor(kBlack)
    me0_h_seg_rate_frac_vfat_window.SetLineWidth(2)
    me0_h_seg_rate_frac_vfat_window.SetLineColor(kBlack)
    latex.DrawLatex(0.9, 0.91,plot_text1.c_str())
    latex.DrawLatex(0.37, 0.91,plot_text2.c_str())
    ...
    c5.SetGrid()
    c5.DrawFrame(0, 0, 9, 0.05, "#eta PartitionAverage Fraction per VFAT")
    ...
    me0_h_seg_rate_frac_vfat_digihits.SetMarkerColor(kBlack)
    me0_h_seg_rate_frac_vfat_digihits.SetLineWidth(2)
    me0_h_seg_rate_frac_vfat_digihits.SetLineColor(kBlack)
    latex.DrawLatex(0.9, 0.91,plot_text1.c_str())
    latex.DrawLatex(0.37, 0.91,plot_text2.c_str())
