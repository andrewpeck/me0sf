...
architecture behavioral of x_prt_deghost_qual is
  ...
  signal better_vectors : range_vector_arr;
  signal better_vectors_padded : range_vector_padded_arr;
  ...
  -- Function to compare every virtual segment with nearby real segments. The corresponding bit is 1 is the virtual segment is better, and 0 if the real segment is better.
  function get_BV(v_seg : segment_t; r_segs : segment_list_t (0 to 5)) return std_logic_vector is    
    variable out_bits : std_logic_vector (0 to 5);
  ...
  begin
    ...
    for y in 0 to NUM_FINDERS-N_X_PRTS-1 loop
      for x in 0 to N_SEGS_PRT-1 loop
        v_kill_bits(0) := range_vectors(y*(N_SEGS_PRT+2) + x)(5) and better_vectors(y*(N_SEGS_PRT+2) + x)(5);
        v_kill_bits(1) := range_vectors(y*(N_SEGS_PRT+2) + x + 1)(4) and better_vectors(y*(N_SEGS_PRT+2) + x + 1)(4);
        v_kill_bits(2) := range_vectors(y*(N_SEGS_PRT+2) + x + 2)(3) and better_vectors(y*(N_SEGS_PRT+2) + x + 2)(3);
        
        v_kill_bits(3) := range_vectors((y+1)*(N_SEGS_PRT+2) + x)(2) and better_vectors((y+1)*(N_SEGS_PRT+2) + x)(2);
        v_kill_bits(4) := range_vectors((y+1)*(N_SEGS_PRT+2) + x + 1)(1) and better_vectors((y+1)*(N_SEGS_PRT+2) + x + 1)(1);
        v_kill_bits(5) := range_vectors((y+1)*(N_SEGS_PRT+2) + x + 2)(0) and better_vectors((y+1)*(N_SEGS_PRT+2) + x + 2)(0);
  ...
  x_prts : for y in 0 to N_X_PRTS-1 generate
    v_seg : for x in 0 to N_SEGS_PRT-1 generate
      ...
      begin
        ...
        better_vectors(N_SEGS_PRT*y + x) <= get_BV(v_seg, r_segs);
    end generate;
  end generate;
  
  range_vectors_padded <= pad_RV(range_vectors);
  better_vectors_padded <= pad_RV(better_vectors);
